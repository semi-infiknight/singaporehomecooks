...
## Singapore Home Cooks overlay

**Upstream:** [expo/skills `eas-simulator`](https://github.com/expo/skills/tree/main/plugins/expo/skills/eas-simulator) (installed under `.agents/skills/eas-simulator/`).

| Item | Value |
|------|--------|
| Expo account | `darksend` (Admin) — auth via `EXPO_TOKEN` / `EXPO_ACCESS` in Cursor secrets |
| Customer app dir | `apps/mobile-customer` · bundle `com.singaporehomecooks.customer` · EAS `@darksend/shc-customer` |
| Cook app dir | `apps/mobile-cook` · bundle `com.singaporehomecooks.cook` · EAS `@darksend/shc-cook` |
| iOS sim build profile | `ios-simulator` in each app's `eas.json` |
| Repo helper script | `bash scripts/eas-ios-cloud-sim.sh` (guards + build + try `simulator:start`) |
| Metro (Mode C) | Customer `:8081`, cook `:8082` — `bash scripts/start-mobile-dev.sh` |
| API | Railway Medusa only (`config/railway-client.json` / `.env.local`) — no localhost Medusa |

**Latest customer `ios-simulator` artifact (2026-07-19):**

- Build: https://expo.dev/accounts/darksend/projects/shc-customer/builds/57ecbd6e-30c2-4660-b2ec-62a9b9eaf773
- Tarball: `https://expo.dev/artifacts/eas/n8wVAhrGELVVcSoaXBhQtL8fwwI8Y4OOqehL8NFr69A.tar.gz`

Re-check with:

```bash
cd apps/mobile-customer
npx --yes eas-cli@latest build:list --platform ios --profile ios-simulator --status finished --limit 1 --non-interactive --json
```

**Cloud VM:** Linux — no local `xcrun simctl`. Use EAS Simulator (this skill), not `scripts/verify-ios-apps.sh`. Android emulator on the VM is separate (`adb` + debug APK).

**Gitignore:** `.env.eas-simulator` holds session tokens — never commit (root `.gitignore`).
