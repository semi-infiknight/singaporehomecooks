...
## Mandatory cleanup (every infra session)

**Never leave orphans for the user to clean up.** After any Railway create/change/deploy:

1. Run `pnpm railway:wire` — `${{Service.VAR}}` references so the canvas shows Postgres, Redis, minio links.
2. Run `pnpm railway:cleanup` — delete detached volumes, duplicate databases, abandoned services.
3. Verify with `railway service list --json` and `curl <medusa>/health`.

Common orphans from recreate loops: extra minio volumes, duplicate Redis (e.g. `Redis` + `Redis-xWuE`), offline empty services. Check which DB/cache medusa actually references before deleting duplicates.
