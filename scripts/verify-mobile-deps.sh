...
if grep -E '^[[:space:]]*react-native:[[:space:]]' "$ROOT/pnpm-workspace.yaml" \
  | grep -v 'mobile>' >/dev/null 2>&1; then
...
if grep -qE '@babel/plugin-transform-react-jsx.*\^8' "$ROOT/apps/mobile-customer/package.json" \
  || grep -qE '@babel/plugin-transform-react-jsx.*\^8' "$ROOT/apps/mobile-cook/package.json"; then
...
if grep -q '^apps/mobile-cook/ios/' "$ROOT/.easignore" 2>/dev/null \
  || grep -q '^apps/mobile-customer/ios/' "$ROOT/.easignore" 2>/dev/null; then
...
if ! grep -q 'android/app/build' "$ROOT/.easignore" 2>/dev/null; then
...
for app in apps/mobile-customer apps/mobile-cook; do
  ...
  if ! grep -q 'ITSAppUsesNonExemptEncryption' "$plist" 2>/dev/null \
    || ! grep -A1 'ITSAppUsesNonExemptEncryption' "$plist" | grep -q '<false/>'; then
  ...
  if ! grep -q '"ITSAppUsesNonExemptEncryption": false' "$ROOT/$app/app.json" 2>/dev/null; then
