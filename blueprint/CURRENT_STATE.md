...
| **Discover** | Admin-managed customer browse via `GET /store/shc/customer-config` (categories, promos, occasions, copy, chips, thresholds). SHC Ops → Catalog. Business rules via `GET/POST /admin/shc/business-rules` + SHC Ops → Controls. Cook-owned: listings, `collection_instructions`, story, images. |
