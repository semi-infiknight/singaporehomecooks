...
**Last Updated:** 2026-07-24 — `shc_product_meta` persists cook-provided name/description/price_cents; `heritage_note` dropped (dish story = `description`); all custom tables have Medusa modules/migrations; compliance docs route wired.
...
| `shc_product_meta` | product_id, cook_id, **name**, **description**, cuisine, occasion_tags, allergen_tiers (JSON...), halal, calories, calories_confidence, ingredients (JSON...), min_qty, **price_cents**, last_minute_premium_pct, **image_url**, created_at, updated_at | name/price/description persisted for cook-created listings; recipe story uses `description`; image_url prefers server-generated WebP derivative | One cook per product enforced (business-rules + cart) |
...
| `shc_order_meta` | order_id, cook_id, collection_date (YYYY-MM-DD), collection_slot (e.g. "18:00-19:00"), allergen_acked_at, address_released_at, paynow_reference, shc_status, **items** (json snapshot), **total_cents**, **collection_notes**, **customer_collection_lat**, **customer_collection_lng**, **customer_collection_postal_code**, **customer_collection_line1**, origin_request_id, credits_applied_cents, is_corporate, corporate_note, created_at, updated_at | shc_status from 09-order-state enum; items+total snapshot for UI list/detail (dish names, amounts); customer collection point snapshotted at checkout for ops/support | Core state machine — see 09-order-state.md; one-cook, allergen rules |
