...
**Customer chrome:** `SHCCard variant="customer"` or `appearance="customer"` (Gourmeat 1px border + soft shadow on bento tints); `SHCButton appearance="customer"`; `SHCVisualBentoTile appearance="customer"` on profile quick tiles; checkout `CollectionSlotPicker` / `PayNowPanel` / `WalletCard` use Gourmeat borders on customer mobile; cook app keeps neo-brutalist defaults.
