...
type SortKey = "name" | "cook" | "price"

const ShcProductListMirror = () => {
  const [statusFilter, setStatusFilter] = useState("")
  const [sortKey, setSortKey] = useState<SortKey>("name")

  const listingsQ = useQuery({
    queryKey: ["shc-mirror", "listings", statusFilter],
    queryFn: () => {
          ...
          <Table.Body>
            {listingsQ.isLoading && (
              <Table.Row>
                <ShcTableCell colSpan={5}>
                  <Text size="small">Loading cook listings…</Text>
                </ShcTableCell>
              </Table.Row>
            )}
            {!listingsQ.isLoading && listings.length === 0 && (
              <Table.Row>
                <ShcTableCell colSpan={5}>
                  <Text size="small" className="text-ui-fg-subtle">
                    No SHC listings match this filter.
                  </Text>
                </ShcTableCell>
              </Table.Row>
            )}
            {listings.map((l) => (
              <Table.Row key={l.id}>
                ...
                <Table.Cell>
                  <Text size="small">{l.cuisine || "—"}</Text>
                  {l.halal && (
...
export default withShcQuery(ShcProductListMirror)
