...
export async function shareInvoicePdf(res: InvoicePdfResponse, fallbackName = 'invoice.pdf'): Promise<void> {
  ...
  // Sanity: decoded PDF should start with %PDF
  const head = (() => {
    try {
      // small decode check without full Buffer in RN
      const sample = b64.slice(0, 16);
      // atob may exist in RN hermes
  ...
  // Fallback: RN Share (iOS can open file URL)
